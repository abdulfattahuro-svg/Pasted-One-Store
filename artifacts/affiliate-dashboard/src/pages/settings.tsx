...
function EmailTemplatesSection() {
  ...
  if (isLoading) return <div className="text-xs text-muted-foreground">Loading templates...</div>;
          ...
          <Tabs defaultValue="general">
            <TabsList className="mb-6 w-full grid grid-cols-4">
              ...
              <TabsTrigger value="commission" className="flex items-center gap-1.5 text-xs">
                <Server className="w-3.5 h-3.5" /> Commission
              </TabsTrigger>
              ...
              <TabsTrigger value="portal" className="flex items-center gap-1.5 text-xs">
                <Trophy className="w-3.5 h-3.5" /> Portal
              </TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="general" className="space-y-5 mt-0">
              ...
              <div className="bg-card border border-border rounded p-5">
                ...
                <p className="text-xs text-muted-foreground mb-3">
                  Use this key in the <code className="bg-secondary px-1 rounded">X-API-KEY</code> header for all API requests.
                </p>
                <div className="flex items-center gap-2">
                  ...
                  <button data-testid="button-toggle-api-key" type="button" onClick={() => setShowApiKey(s => !s)} className="p-2 rounded border border-border hover:bg-accent transition-colors text-muted-foreground">
                    {showApiKey ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
                  </button>
                  <button data-testid="button-copy-api-key" type="button" onClick={handleCopyApiKey} className="p-2 rounded border border-border hover:bg-accent transition-colors text-muted-foreground">
              ...
              <div className="bg-card border border-border rounded p-5 space-y-4">
                <div>
                  <h2 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                    <Globe className="w-3.5 h-3.5" /> Program Info
                  </h2>
                  <p className="text-xs text-muted-foreground mt-1">Shown in the affiliate portal and onboarding.</p>
                </div>
                ...
                <FormField control={form.control} name="programDetails" render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-xs">Program Details <span className="text-muted-foreground font-normal">(shown in welcome modal)</span></FormLabel>
                    <FormControl>
            ...
            {/* ── COMMISSION TAB ── */}
            <TabsContent value="commission" className="space-y-5 mt-0">
              <div className="bg-card border border-border rounded p-5 space-y-5">
                <div>
                  <h2 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                    <Server className="w-3.5 h-3.5" /> Affiliate Approval
                  </h2>
                  <p className="text-xs text-muted-foreground mt-1">Control how new affiliate signups are handled.</p>
                </div>

                <FormField control={form.control} name="approvalMode" render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-xs">Approval Mode</FormLabel>
                    <FormControl>
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      {field.value === "auto"
                        ? "New signups are activated immediately after email verification."
                        : "New signups require admin approval before they can access the portal."}
                    </p>
                  </FormItem>
                )} />
              </div>
              ...
              <div className="bg-card border border-border rounded p-5 space-y-5">
                <div>
                  <h2 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                    <Server className="w-3.5 h-3.5" /> Commission Structure
                  </h2>
                  <p className="text-xs text-muted-foreground mt-1">Default commission applied when no product-level override is set.</p>
                </div>
                ...
                <FormField control={form.control} name="commissionValue" render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-xs">
                      Commission Value <span className="text-muted-foreground font-normal">{commissionType === "fixed" ? "(fixed amount)" : "(percentage)"}</span>
                    </FormLabel>
                    <FormControl>
                ...
                <FormField control={form.control} name="holdDays" render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-xs">Hold Period <span className="text-muted-foreground font-normal">(days before commission becomes payable)</span></FormLabel>
            ...
            <TabsContent value="email" className="space-y-5 mt-0">
              <div className="bg-card border border-border rounded p-5 space-y-4">
                <div>
                  <h2 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                    <Mail className="w-3.5 h-3.5" /> Email Provider
                  </h2>
                  <p className="text-xs text-muted-foreground mt-1">Choose how transactional emails are delivered to affiliates.</p>
                </div>
                ...
                {emailProvider === "smtp" && (
                  <div className="space-y-3">
                    ...
                    <FormField control={form.control} name="smtpPass" render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-xs">SMTP Password <span className="text-muted-foreground font-normal">(leave blank to keep existing)</span></FormLabel>
                        <FormControl>
                          <div className="relative">
                ...
                {emailProvider === "resend" && (
                  <div className="space-y-3">
                    <FormField control={form.control} name="resendApiKey" render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-xs">Resend API Key <span className="text-muted-foreground font-normal">(leave blank to keep existing)</span></FormLabel>
                        <FormControl>
                          <div className="relative">
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      Get your API key at <a href="https://resend.com" target="_blank" rel="noreferrer" className="text-primary underline">resend.com</a>. Free tier includes 3,000 emails/month.
                    </p>
                  </div>
                )}
              </div>
            ...
            {/* ── PORTAL TAB ── */}
            <TabsContent value="portal" className="space-y-5 mt-0">
              <LeaderboardSection />

              {/* API Endpoints Reference */}
              <div className="bg-card border border-border rounded p-5">
                <h2 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-3 flex items-center gap-1.5">
                  <Shield className="w-3.5 h-3.5" /> API Endpoints Reference
                </h2>
                <p className="text-xs text-muted-foreground mb-3">
                  Use these endpoints to integrate with your platform. Include the <code className="bg-secondary px-1 rounded">X-API-KEY</code> header on all requests.
                </p>
                <div className="space-y-2 font-mono text-[10px] text-muted-foreground">
                  {[
                    ...
                    <div key={path} className="flex items-center gap-2 py-1 border-b border-border/50 last:border-0">
                      <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold flex-shrink-0 ${method === "GET" ? "bg-blue-500/10 text-blue-400" : "bg-amber-500/10 text-amber-400"}`}>
                        {method}
                      </span>
                      <code className="text-foreground flex-shrink-0">{path}</code>
