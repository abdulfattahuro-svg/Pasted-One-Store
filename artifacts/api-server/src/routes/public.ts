...
// ─── Commission trigger for API leads ────────────────────────
async function triggerLeadCommission(lead: typeof leadsTable.$inferSelect, triggerEvent: string) {
  ...
  let commission = ruleValue;
  if (rule.commissionType === "hybrid" && rule.recurringPercentage) {
    commission = ruleValue + Number(rule.recurringPercentage);
  }
...
router.post("/public/leads", async (req, res) => {
  ...
  const { affiliateCode, fullName, phone, email, notes, source, metadata } = req.body as Record<string, unknown>;
